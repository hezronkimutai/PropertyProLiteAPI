...
function userValidator (res, inputs) {
  ...
  const validFirstName = inputs.first_name.toLowerCase().match(stringPattern)
  const validsecond_name = inputs.last_name.toLowerCase().match(stringPattern)
  const validuser_name = inputs.user_name.toLowerCase().match(stringPattern)
  const validEmail = inputs.email.match(emailPattern)
  const validPhoneNumber = inputs.phone_number.match(numericPattern)
